...
class DataLoader:
    ...
    def load_eda_insights(_self):
        """Load EDA insights - FIXED UTF-8 ERROR"""
        try:
            df, encoding = _self._safe_read_csv(_self.config.EDA_INSIGHTS)
            return df
        except FileNotFoundError:
            st.warning("EDA insights not found")
            return None
        except Exception as e:
            st.warning(f"Error loading EDA insights: {str(e)}")
            return None
    ...
    def load_momentum_windows(_self):
        ...
        try:
            ...
            st.warning(f"Error loading momentum windows: {str(e)}")
            return None
