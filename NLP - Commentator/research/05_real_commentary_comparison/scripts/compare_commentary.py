...
try:
    ...
    print("⚠️  spaCy not installed. Installing now...")
...
def extract_entities(text, normalized_text):
    ...
    try:
        # Players (from normalized text - already extracted)
        for canonical, variations in PLAYER_VARIATIONS.items():
        ...
        actions = [token.lemma_ for token in doc 
                   if token.pos_ == 'VERB' 
                   and token.lemma_ not in ['be', 'have', 'do', 'will', 'can', 'may']]
        entities['actions'] = list(set(actions))[:10]  # Limit to 10 unique actions
        ...
        for chunk in doc.noun_chunks:
            ...
            if any(kw in chunk_lower for kw in location_keywords):
                entities['locations'].append(chunk.text)
...
def calculate_entity_overlap(entities1, entities2):
    ...
    try:
        ...
        if entities1['locations'] and entities2['locations']:
            location_match = any(
                loc1.lower() in loc2.lower() or loc2.lower() in loc1.lower()
                for loc1 in entities1['locations']
                for loc2 in entities2['locations']
            )
...
# Load sentence transformer model (global to avoid reloading)
...
def compare_minute(minute, our_sequences, real_commentary):
    ...
    real_entities = extract_entities(real_commentary, real_normalized)
    
    for seq_id, seq_text, seq_score in our_sequences:
        # Normalize our sequence
        our_normalized = normalize_text(seq_text)
        our_entities = extract_entities(seq_text, our_normalized)
...
def process_match(match_name, match_id, our_commentary_path, real_commentary_path):
    ...
    print(f"✓ Our commentary: {len(our_df)} events")
    print(f"✓ Real commentary: {len(real_df)} entries")
    
    # Get sequences with commentary
    ...
    for minute in sorted(real_df['minute'].unique()):
        ...
        real_commentary_list = real_entries['commentary'].tolist()
