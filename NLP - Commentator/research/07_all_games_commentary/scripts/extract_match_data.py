...
def calculate_tournament_stats_before_match(matches_df, events_df, team_name, match_date):
    ...
    team_events = events_df[events_df['match_id'].isin(team_match_ids)].copy()
    ...
    team_events.loc[:, 'player_name'] = team_events.apply(get_player_name, axis=1)
    
    # Count goals by player
    goals = team_events[team_events['event_type'] == 'Shot'].copy()
    
    # Ensure player_name exists in goals
    if 'player_name' not in goals.columns:
        goals['player_name'] = goals.apply(get_player_name, axis=1)
    ...
    goals.loc[:, 'is_goal'] = goals.apply(is_goal, axis=1)
    
    # Filter and count
    goal_shots = goals[goals['is_goal'] == True]
    if len(goal_shots) > 0 and 'player_name' in goal_shots.columns:
        player_goals = goal_shots.groupby('player_name').size().to_dict()
    else:
        player_goals = {}
...
def enrich_event_data(events_df, team_stats, player_goals, match_info, team1_name, team2_name):
    ...
    for event_idx, (idx, row) in enumerate(events_list):
        ...
        # Handle own goals (Own Goal Against = player's team concedes)
        if event['event_type'] == 'Own Goal Against':
            if event['team_name'] == team1_name:
                # Team 1 player scored own goal, Team 2 gets the goal
                team2_score += 1
                team2_goals_in_match.append({'player': f"OG: {event['player_name']}", 'minute': event['minute'], 'second': event['second']})
            elif event['team_name'] == team2_name:
                # Team 2 player scored own goal, Team 1 gets the goal
                team1_score += 1
                team1_goals_in_match.append({'player': f"OG: {event['player_name']}", 'minute': event['minute'], 'second': event['second']})
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python extract_match_data.py <match_id>")
        print("Example: python extract_match_data.py 3930158  # Germany vs Scotland")
        sys.exit(1)
    ...
    # Save to CSV
    output_file = os.path.join(SCRIPT_DIR, '..', 'data', f'match_{match_id}_detailed_commentary_data.csv')
