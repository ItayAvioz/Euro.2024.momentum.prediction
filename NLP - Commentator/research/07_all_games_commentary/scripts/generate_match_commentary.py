...
def format_shot_commentary(row, df, idx):
    ...
    if is_goal:
        ...
        # Calculate new score - use dynamic team names
        # team_a is mapped to spain_score, team_b to england_score
        if team == TEAM_A_NAME:
        ...
        # Add score - SHOW BOTH SCORES CORRECTLY using actual team names
        ...
        if player_match_goals_now == 2:
            commentary += f"That's {player}'s second goal of the match! A brace! "
        elif player_match_goals_now == 3:
            commentary += f"That's {player}'s THIRD goal of the match! A hat-trick! "
        ...
        if score_diff == 2:
            commentary += "A crucial two-goal lead! "
...
def format_own_goal_commentary(row, df, idx):
    """Generate commentary for own goals"""
    global TEAM_A_NAME, TEAM_B_NAME
    ...
    if event_type == 'Own Goal Against':
        # This is the player who scored the own goal
        if pd.notna(player):
            # Determine which team benefits
            if team == TEAM_A_NAME:
                # Team A player scored own goal, Team B benefits
                beneficiary = TEAM_B_NAME
                new_spain = spain_score
                new_england = england_score + 1
            else:
                # Team B player scored own goal, Team A benefits
                beneficiary = TEAM_A_NAME
                new_spain = spain_score + 1
                new_england = england_score
            
            commentary = f"💥 OWN GOAL! {player} puts the ball into his own net! "
            commentary += f"Cruel for {team}, but {beneficiary} won't mind! "
            ...
            if new_spain > new_england:
                ...
                commentary += f"We're level at {new_spain}-{new_england}! "
            
            return commentary
        else:
            return f"💥 OWN GOAL! An own goal extends the lead!"
    
    elif event_type == 'Own Goal For':
        # This marks which team benefits (but doesn't have player info)
        return ""  # Skip this, commentary comes from "Own Goal Against"
...
def add_period_commentary(row):
    ...
    # GAME START - Use dynamic team names
    if minute == 0 and period == 1 and event_type == 'Pass' and 'Kick Off' in str(play_pattern):
        team_a_wins = row.get('team_tournament_wins', 0) if row['team_name'] == TEAM_A_NAME else 0
        team_a_goals = row.get('team_tournament_goals', 0) if row['team_name'] == TEAM_A_NAME else 0
        team_a_conceded = row.get('team_tournament_conceded', 0) if row['team_name'] == TEAM_A_NAME else 0
        
        return f"🏆 THE EURO 2024 MATCH IS UNDERWAY! Welcome for this clash between {TEAM_A_NAME} and {TEAM_B_NAME}. Both teams have fought hard to reach this stage of Euro 2024. We're all set for kick-off! "
    ...
    # SECOND HALF START
        ...
        if spain_score > england_score:
            return f"⚽ THE SECOND HALF IS UNDERWAY! {TEAM_A_NAME} lead {spain_score}-{england_score} at the break. Can {TEAM_B_NAME} find a way back? 45 minutes to go! "
        elif england_score > spain_score:
            return f"⚽ THE SECOND HALF IS UNDERWAY! {TEAM_B_NAME} lead {england_score}-{spain_score} at the break. Can {TEAM_A_NAME} respond in the second half? Everything to play for! "
        else:
    ...
    # STOPPAGE TIME
        ...
        england_score = row.get('england_score', 0)
        if spain_score != england_score:
            return f"⏱️ WE'RE INTO STOPPAGE TIME! Seconds remaining in this crucial Euro 2024 match! "
        else:
...
def generate_event_commentary(df):
    ...
    for idx, row in df.iterrows():
        ...
        elif event_type == 'Own Goal Against' or event_type == 'Own Goal For':
            commentary = format_own_goal_commentary(row, df, idx)
...
def create_sequences(df):
    ...
    for idx, row in df.iterrows():
        ...
        # Check if this is a key event that ends a sequence
        is_key_event = event_type in ['Shot', 'Own Goal Against', 'Own Goal For', 'Foul Committed', 'Substitution']
        
        # Add current event to sequence first
        current_sequence.append(idx)
        
        # Start new sequence if:
        # 1. Possession changes
        # 2. Key event occurred (goal, own goal, sub, etc.)
        # 3. Sequence gets too long (>10 events)
        should_end_sequence = (
            (possession_team != current_possession_team and current_possession_team is not None) or
            is_key_event or
            len(current_sequence) >= 10
        )
        
        if should_end_sequence:
            if current_sequence:
                sequences.append({
...
def create_narrative_flow(df, event_indices):
    ...
    for i, idx in enumerate(selected_indices):  # Use selected events (build-up + goals)
        try:
            ...
            if i == 0:
                ...
                # Different player, same team: Add comma before new player
                elif team == last_team and pd.notna(team):
                    # Add comma to separate actions by different players
                    if len(narrative_parts) > 0 and not narrative_parts[-1].endswith(','):
    ...
    # Fallback: If no narrative created, use all non-receive events with proper formatting
    if len(narrative_parts) == 0:
        # Try again with ALL events (not just selected) - create FULL narrative
        last_player_fallback = None
        last_team_fallback = None
        ...
        for idx in event_indices:
            try:
                ...
                # Skip Ball Receipt* but include everything else
                if event_type == 'Ball Receipt*':
                    continue
                
                commentary = event.get('event_commentary', '')
                if pd.isna(commentary) or len(str(commentary).strip()) == 0:
                    continue
                    
                # Remove period markers from commentary (don't skip the event!)
                commentary = str(commentary)
                if '🏆 THE EURO 2024 FINAL' in commentary:
                    # Remove everything up to and including the period marker
                    parts = commentary.split('!')
                    if len(parts) > 1:
                        # Take everything after the last '!' which is the actual event commentary
                ...
                if len(narrative_parts) == 0:
                    # First event: add as-is
                    narrative_parts.append(str(commentary))
                else:
                    # Check if same player/team
                    if player == last_player_fallback and team == last_team_fallback and pd.notna(player):
                        # Same player: remove name repetition
                        if pd.notna(player) and str(commentary).startswith(str(player)):
                            action = str(commentary)[len(str(player)):].lstrip(' ,')
                            if action:
                                narrative_parts.append(action)
                        else:
                            narrative_parts.append(str(commentary))
                    elif team == last_team_fallback and pd.notna(team):
                        # Different player, same team: add comma before
                        if not narrative_parts[-1].endswith(','):
                            narrative_parts[-1] = narrative_parts[-1].rstrip() + ','
                        narrative_parts.append(str(commentary))
                    else:
                        # Different team: new sentence
                        narrative_parts.append(". " + str(commentary))
                
                last_player_fallback = player
                last_team_fallback = team
            except:
                continue
        
        # Still empty? Use last event as absolute fallback
        if len(narrative_parts) == 0:
            try:
                last_event = df.loc[event_indices[-1]]
                player = last_event.get('player_name', 'Player')
                event_type = last_event.get('event_type', 'plays')
                narrative_parts.append(f"{player} {event_type.lower()}")
            except:
                narrative_parts.append("Action continues")
    
    # Join parts with space, clean up
    result = ' '.join(narrative_parts)
    # Clean up double spaces, extra commas, and extra periods
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python generate_semi_final_commentary.py <match_id>")
    ...
    # Load enriched data
    input_file = os.path.join(SCRIPT_DIR, '..', 'data', f'match_{match_id}_detailed_commentary_data.csv')
    if not os.path.exists(input_file):
        print(f"ERROR: File not found: {input_file}")
        print("Run extract_semi_final_data.py first!")
        sys.exit(1)
    
    df = pd.read_csv(input_file)
    
    print(f"Loaded {len(df)} events for match {match_id}")
    ...
    if len(unique_teams) == 2:
        ...
        print(f"  ERROR: Expected 2 teams, found {len(unique_teams)}: {unique_teams}")
        sys.exit(1)
    ...
    # Normalize column names - rename team-specific score columns to spain_score/england_score
    # This allows the existing templates to work without modification
    score_cols = [c for c in df.columns if '_score' in c and c not in ['score_diff']]
    if len(score_cols) == 2:
        # Rename first team to spain_score, second to england_score (generic names for templates)
        rename_map = {
            score_cols[0]: 'spain_score',
            score_cols[1]: 'england_score'
        }
        df = df.rename(columns=rename_map)
        print(f"  Normalized score columns: {score_cols[0]} -> spain_score, {score_cols[1]} -> england_score")
    ...
    print(f"\n[SUCCESS] Rich commentary saved to: {output_file}")
