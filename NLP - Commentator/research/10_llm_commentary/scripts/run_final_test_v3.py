...
def analyze_multiple_substitutions(minute_df):
    ...
    for team in team_counts:
        ...
        players_by_team[team] = team_subs['player_name'].tolist()
...
def detect_all_important_events(minute_df, detection_info):
    ...
    # 6. If NO important events, detect main event
    if len(events) == 0:
...
def detect_event_chain(minute_df, detected_type, main_row, detection_info):
    """Detect related events that form a chain."""
    chain = {'has_chain': False, 'related_events': []}
    
    if detected_type == 'Goal':
        scorer = main_row.get('player_name', '')
        prev_event_type = main_row.get('previous_event_type', '')
        prev_player = main_row.get('previous_player', '')
        
        if prev_event_type == 'Pass' and prev_player and pd.notna(prev_player):
            assister = str(prev_player).strip()
            if assister and assister != scorer:
                chain['has_chain'] = True
                chain['related_events'].append({'type': 'Assist', 'player': assister})
